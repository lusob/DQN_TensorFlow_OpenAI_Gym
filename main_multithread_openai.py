...
class deep_atari:
    def __init__(self,params):
        ...
        self.DB = ReplayMemory(self.params)
        self.env = gym.make(self.params['env_name'])
        # TODO:  Commented to  use breakout trained networks with 4 actions instead of 6
        self.params['num_act'] = self.env.action_space.n
        #self.params['num_act'] = 4
        self.build_net()
        self.training = True
    ...
    def start(self):
        self.env.monitor.start(MONITOR_FILES_PATH, force=True, video_callable=lambda x: self.params['record_eval'] == 'y' and self.record_video)
        self.reset_game()
        self.step = 0
        ...
        if self.train_cnt > 0 :
            ...
            try:
                self.log_train = open('log_training_openai_'+self.params['network_type']+'.csv','a')
            except:
                self.log_train = open('log_training_openai_'+self.params['network_type']+'.csv','w')
                self.log_train.write('step,epoch,train_cnt,avg_reward,avg_q,epsilon,time\n')

            try:
                self.log_eval = open('log_eval_openai_'+self.params['network_type']+'.csv','a')
            except:
                self.log_eval = open('log_eval_openai_'+self.params['network_type']+'.csv','w')
        ...
        while self.step < (self.params['steps_per_epoch'] * self.params['num_epochs'] * self.params['learning_interval'] + self.params['train_start']):
            ...
            self.action_idx, self.maxQ = self.select_action(self.state_proc)
            self.state, self.reward, self.terminal, _ = self.env.step(self.action_idx)
            if self.params['visualize'] and not self.training:
                self.env.render()
    ...
    def reset_game(self):
        self.state_proc = np.zeros((84,84,4)); self.terminal = False; self.reward = 0
    ...
    def select_action(self,st):
        if np.random.rand() > self.params['eps']:
            ...
            #act_idx = np.random.randint(0,self.params['num_act'])
            self.lock.acquire()
            Q_pred = self.sess.run(self.qnet.y, feed_dict = {self.qnet.x: np.reshape(st, (1,84,84,4))})[0]
            self.lock.release()
            return act_idx, Q_pred[act_idx]
...
if __name__ == "__main__":
    ...
    for i in range(1,len(sys.argv),2):
        if sys.argv[i] == '-weight' :params['ckpt_file'] = sys.argv[i+1]
        ...
        elif sys.argv[i] == '-record_eval' : params['record_eval'] = sys.argv[i+1]
        else :
            ...
            print '        -record_eval (y or n)'
            print '        -only_eval (y or n)'
